...
(function($) {
    ...
    $.fn.rssfeed1 = function (url, options, fn) {
        ...
        for (var i = 0; i < data.channel.item.length; i++) {
            ...
            if (options.media && xmlEntries.length > 0) {
                ...
                if (xmlMedia.length > 0) {
                    //html += '<a class="news_thumb" href="#">'
                    for (var m = 0; m < xmlMedia.length; m++) {
                        var xmlUrl = xmlMedia[m].getAttribute("url");
                        var xmlType = xmlMedia[m].getAttribute("type");
                        var xmlSize = xmlMedia[m].getAttribute("length");
                        //alert("xmlType = " + xmlType);
                        if (xmlType === "img/jpg") {
                            html += '<li class="thumbs_li" style="background:url(' + xmlUrl + '); background-size: 392px 201px;"><article>';
            ...
            //html += '<h3><a href="' + entry.link + '" title="' + feeds.title + '">' + entry.title + '</a></h3>';
            ...
            var longDescription = entry.longtext;

            //article body
            if (options.content) {
                if (entry.encoded) {
                    html += '<p class="desc">' + shorten(entry.encoded, 130); + '</p>';
                    html += '<div class="long_desc">' + longDescription + '</div>';
                } else {
                    html += '<p class="desc">' + shorten(entry.description, 130); + '...' + '</p>';
                    html += '<div class="long_desc">' + longDescription + '</div>';
                }
            }
